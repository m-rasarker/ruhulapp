...
class Alertpractic extends StatefulWidget {
  const Alertpractic({super.key});

  @override
  State<Alertpractic> createState() => _AlertpracticState();
}


class _AlertpracticState extends State<Alertpractic> {
  int age=0;
...
void alert(){
  ...
  void showAlert(){
    showDialog(context: context, builder: (context) => AlertDialog(
      title: Text('This the Alert'),
      content: Text('Are you Sure to go forward?'),
      actions: [
      TextButton(onPressed: (){
        Navigator.pop(context);
      }, child: Text('No')),
